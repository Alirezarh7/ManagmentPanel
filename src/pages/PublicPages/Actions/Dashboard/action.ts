...
export const dashboardActions = {
    ...
    toggleLogoutConfirm: (visible: boolean): AppAction<KnownAction> => async (dispatch, getState) => {
        dispatch({
    ...
    setUpdatePerson: (automaticlyLogout: any): AppAction<KnownAction> => async (dispatch, getState) => {
        if (localStorage.getItem(shareData.CONSTANT.REPLACE_SSN) && localStorage.getItem(shareData.CONSTANT.REPLACE_BIRTH_DATE)) {
            try {
                const result = await API.post('/People/CreatePerson',
                    localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY) ? shareData.validDataReplaceSsn(JSON.parse(localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY) as string).profile) :
                        shareData.validDataReplaceSsn(JSON.parse(localStorage.getItem(shareData.CONSTANT.ORGANIZATION_STORAGE_KEY) as string).profile));
                dispatch({
                    type: DashboardActionTypes.CreatePersonSuccess
                });
                const access = localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY) ? JSON.parse(localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY) as string) : JSON.parse(localStorage.getItem(shareData.CONSTANT.ORGANIZATION_STORAGE_KEY) as string);
                ...
                localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY) ? localStorage.setItem(shareData.CONSTANT.GOV_STORAGE_KEY, JSON.stringify(access)) : localStorage.setItem(shareData.CONSTANT.ORGANIZATION_STORAGE_KEY, JSON.stringify(access));
            } catch (error) {
                automaticlyLogout();
            }
        }
    ...
    setCreateToken: (validData: any, callInSuccessCallback: boolean, automaticlyLogout: any, accessToken: string): AppAction<KnownAction> => async (dispatch, getState) => {
        dispatch({type: DashboardActionTypes.CreateToken});
        try {
            const resultToken = await axios.post(`${currentBaseUrl}/User/Token`, {nationalCode: validData.nationalCode}, {
            ...
            if (resultToken.status === 200) {
                ...
                if (localStorage.getItem(shareData.CONSTANT.GOV_STORAGE_KEY)) {
                    ...
                    localStorage.setItem(shareData.CONSTANT.GOV_STORAGE_KEY, JSON.stringify(access));
                    callInSuccessCallback && window.location.reload();
                } else if (localStorage.getItem(shareData.CONSTANT.ORGANIZATION_STORAGE_KEY)) {
                    const updatedTime = new NDate().subDays(-1);
                    updatedTime.date.setHours(new Date().getHours() + 4);
                    const updateExpiredGovSsoTime = new Date(updatedTime.date).getTime();

                    const access = JSON.parse(localStorage.getItem(shareData.CONSTANT.ORGANIZATION_STORAGE_KEY) as any);
                    if (localStorage.getItem(shareData.CONSTANT.REPLACE_SSN) && localStorage.getItem(shareData.CONSTANT.REPLACE_BIRTH_DATE)) {
                        result.data.nationalCode = JSON.parse(localStorage.getItem(shareData.CONSTANT.REPLACE_SSN) as any);
                        result.data.birthDate = JSON.parse(localStorage.getItem(shareData.CONSTANT.REPLACE_BIRTH_DATE) as any);
                    }
    ...
    showRequestErrors: (error: any): AppAction<KnownAction> => async (dispatch, getState) => {
        if (error.response && error.response.status) {
            if (error.response.status == 401)
                ...
                dashboardActions.pushCommonAlert('403')(dispatch, getState)
