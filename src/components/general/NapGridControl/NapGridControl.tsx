...
function useGridControl(): any {
	...
	function onCellClicked(params: any): void {
		if (params.value?.hasMenu) {
			...
			const cell =
				params.api.context.beanWrappers.rowRenderer.beanInstance.rowCompsByIndex[rowIndex].getRenderedCellForColumn(column);
			cell.beans.contextMenuFactory.showMenu(rowNode, column, value, mouseEvent);
		}
	}
	function publicContextMenu(): any {
		return ['export', 'copy', 'separator', 'chartRange'];
	}
	function checkAccessControl(data: { service?: string; controller?: string; action: string }, claims: any): boolean {
		...
		if (
			claims.roles.some(
				(x: any) =>
					x.toLocaleLowerCase() == 'OAuthAdminAdministrator'.toLocaleLowerCase() ||
					x.toLocaleLowerCase() == 'admin'.toLocaleLowerCase()
			)
		)
			hasAccess = false;
			...
			data.controller &&
			claims.controllers.some((x: any) => x.toLocaleLowerCase() == data.controller?.toLocaleLowerCase())
		)
			hasAccess = false;
		else if (claims.actions.some((x: any) => x.toLocaleLowerCase() == data.action.toLocaleLowerCase())) hasAccess = false;
		return hasAccess;
	}
